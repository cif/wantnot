...
function TransactionsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'list' | 'category' | 'project'>('category');
  ...
  const fetchTransactionsForMonth = async (monthYear: string) => {
    try {
      ...
      const response = await fetch(`/api/transactions?month=${monthYear}&includeHidden=${showHidden}`, {
  ...
  const fetchUncategorized = async () => {
    try {
      ...
      const response = await fetch(`/api/transactions/uncategorized?includeHidden=${showHidden}`, {
        headers: { 'Authorization': `Bearer ${idToken}` },
      });
  ...
  const [allTransactionsForMonths, setAllTransactionsForMonths] = useState<any[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      const [categoriesRes, projectsRes, allTransactionsRes] = await Promise.all([
  ...
  const filteredTransactions = useMemo(() => {
    ...
    if (selectedCategoryId === 'uncategorized') {
      ...
      filtered = filtered.filter(txn => txn.categoryId === selectedCategoryId);
    }
  ...
  const categorySpending = useMemo(() => {
    ...
    const relevantTransactions = filteredTransactionsForCharts.filter(txn => !txn.pending && !txn.isTransfer);

    // Group by category
    const categoryMap = new Map<string, { category: any; transactions: any[]; total: number }>();
    ...
    relevantTransactions.forEach(txn => {
      ...
      if (!categoryMap.has(categoryId)) {
        ...
        categoryMap.set(categoryId, {
          category: category || { id: 'uncategorized', name: 'Uncategorized', color: '#9CA3AF', isIncome: false },
          transactions: [],
          total: 0
        });
      }
  ...
  const projectSpending = useMemo(() => {
    ...
    // Group by project
    const projectMap = new Map<string, { project: any; transactions: any[]; total: number }>();
    ...
    relevantTransactions.forEach(txn => {
      ...
      if (!projectMap.has(projectId)) {
        ...
        projectMap.set(projectId, {
          project: project || { id: 'untagged', name: 'Untagged', color: '#9CA3AF' },
          transactions: [],
          total: 0
        });
      }
    ...
    const totalSpending = allProjects.reduce((sum, { total }) => sum + total, 0);
    ...
    <div className="p-4 max-w-7xl mx-auto">
      ...
      <div className="mb-4 grid grid-cols-3 gap-3">
        <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-200">
          ...
          <p className="text-lg font-bold text-green-600 tabular-nums">{formatCurrency(monthTotals.income, { showSign: true })}</p>
          {monthTotals.hasPending && monthTotals.pendingIncome > 0 && (
            <p className="text-xs text-yellow-600 mt-0.5 tabular-nums">{formatCurrency(monthTotals.pendingIncome, { showSign: true })} pending</p>
          )}
        </div>
        <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-200">
          <p className="text-xs text-gray-600 mb-1">Expenses</p>
          <p className="text-lg font-bold text-gray-700 tabular-nums">-{formatCurrency(monthTotals.expenses)}</p>
          {monthTotals.hasPending && monthTotals.pendingExpenses > 0 && (
            <p className="text-xs text-yellow-600 mt-0.5 tabular-nums">-{formatCurrency(monthTotals.pendingExpenses)} pending</p>
          )}
        </div>
        <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-200">
          <p className="text-xs text-gray-600 mb-1">Net</p>
          <p className={`text-lg font-bold tabular-nums ${monthTotals.net >= 0 ? 'text-green-600' : 'text-gray-700'}`}>
          ...
          {monthTotals.hasPending && (
            <p className="text-xs text-yellow-600 mt-0.5 tabular-nums">
              ...
              {formatCurrency(Math.abs(monthTotals.pendingIncome - monthTotals.pendingExpenses))} pending
            </p>
          )}
        </div>
      </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <p className="text-xs font-medium text-gray-900">
                          {new Date(txn.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' })}
                        </p>
                      </div>
      ...
      {/* By Category View */}
      {activeTab === 'category' && !loading && filteredTransactions.length > 0 && (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              {(() => {
                const totalBudget = categorySpending.expenses
                  ...
                  .reduce((sum, { category }) => sum + parseFloat(category.budgetLimit), 0);
                ...
                if (totalBudget > 0) {
                  const overallPercentage = (totalBudgetedSpending / totalBudget) * 100;
                    ...
                    <div className="mb-4 pb-4 border-b border-gray-200">
                      ...
                      <div className="relative">
                        ...
                        <p className={`text-xs mt-1 ${
                          overallPercentage > 100 ? 'text-red-600 font-medium' : 'text-gray-500'
                        }`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {categorySpending.expenses.map(({ category, total, transactions }) => {
                  ...
                  const barWidth = budgetPercentage !== null
                    ...
                    >
                      ...
                      {category.budgetLimit ? (
                        <div className="relative">
                          ...
                          <p className={`text-xs mt-1 ${
                            budgetPercentage > 100 ? 'text-red-600' : 'text-gray-500'
                          }`}>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="w-full bg-gray-100 rounded-full h-8 overflow-hidden flex">
                {categorySpending.expenses.map(({ category, total }, idx) => {
                  const percentage = (total / categorySpending.totalExpenses) * 100;
                  return (
                    <div
                      key={category.id}
          ...
          {categorySpending.income.length > 0 && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-600 mt-0.5 tabular-nums">
                  Total: {formatCurrency(categorySpending.totalIncome, { showSign: true })}
                </p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {categorySpending.income.map(({ category, total, transactions }) => {
                  // Calculate percentage of budget used (for bar width)
                  const budgetPercentage = category.budgetLimit
                    ? (total / parseFloat(category.budgetLimit)) * 100
                    : null;
          ...
          {categorySpending.expenses.length === 0 && categorySpending.income.length === 0 && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center">
              <p className="text-gray-600">No categorized transactions found for this period.</p>
            </div>
          )}
        </div>
      )}
      ...
      {/* By Project View */}
      {activeTab === 'project' && !loading && filteredTransactions.length > 0 && (
        <div className="space-y-6">
          {projectSpending.projects.length > 0 ? (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                {projectSpending.projects.map(({ project, total, transactions }, idx, arr) => {
                  ...
                  const percentageOfTotal = (total / projectSpending.totalSpending) * 100;
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center">
              <p className="text-gray-600">No project-tagged transactions found for this period.</p>
            </div>
          )}
        </div>
      )}
